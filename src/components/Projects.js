...
const Projects = () => {
  ...
  return (
    ...
    <motion.div  animate={{y:['-100%','0%'],opacity:['0%',"100%"]}} transition={{delay:1,duration:0.5}} className='lg:text-7xl text-4xl md:text-5xl font-ubuntu font-extrabold  '>Projects</motion.div>
    <div className='flex gap-24 flex-wrap'>
        {data?.user.projects.map((object)=>{
            return <ProjectCard project={object}/>
        })}
    </div>
